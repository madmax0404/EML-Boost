"""Experiment 19: level-wise engine parity run on OpenML-CTR23.

Identical protocol to Exp 18 (same seeds, same matched XGB/LGB configs,
same OpenML-CTR23 datasets, same early stopping); the only change is
SplitBoost running `tree_growth="levelwise"` instead of the `"nodewise"`
default — this run is the spec's default-flip gate. Same
matched-comparison framework as Exp 17/18:
...
algorithmic confounds, same as Exp 17/18.

Spec: docs/superpowers/specs/2026-07-11-levelwise-growth-design.md

Estimated runtime: ~15-40 min on RTX 3090 post-speedup (Exp 18 took
1-2.5h at nodewise). Run with:
  PYTHONUNBUFFERED=1 uv run python -u experiments/run_experiment19_levelwise_parity.py 2>&1 | tee experiments/experiment19/run.log
"""
...
RESULTS_DIR = Path(__file__).resolve().parent / "experiment19"
...
def _load_ctr23_dataset_names() -> list[str]:
    ...
    """
    # Suite 353 = "OpenML-CTR23 - A curated tabular regression benchmarking suite" (35 tasks).
    # The string alias "OpenML-CTR23" is not accepted by the API as of 2026-04; use numeric ID.
...
def _fit_lgb(X_tr, y_tr, seed):
    ...
    m.n_rounds = m.num_trees()  # expose for logging (early-stopping aware)
    return m, time.time() - start
